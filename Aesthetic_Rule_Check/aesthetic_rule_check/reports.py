...
def render_html(result: EvaluationResult) -> str:
    ...
    deductions = render_deductions(result)
    prompt_suggestions = render_prompt_suggestions(result)
    hard_caps = render_hard_caps(result)
...
<head>
  ...
  <style>
    ...
    .deductions {{ grid-column: 1 / -1; }}
    .deduction {{ border-bottom: 1px solid #edf0f2; padding: 10px 0; }}
    .deduction:last-child {{ border-bottom: 0; }}
    .deduction strong {{ display: inline-block; min-width: 42px; color: #b42318; }}
    .deduction small {{ color: #57606a; display: block; margin-top: 4px; line-height: 1.5; }}
...
def render_batch_html(results: list[EvaluationResult], output_dir: Path) -> str:
    ...
    frequent_deductions = render_frequent_deductions(results)
    frequent_hints = render_frequent_hints(results)
    return f"""<!doctype html>
<html lang="zh-CN">
<head>
  <meta charset="utf-8">
  ...
  <style>
    ...
    .insights {{ display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 16px; margin-bottom: 16px; }}
    section {{ background: white; border: 1px solid #e5e7eb; border-radius: 8px; padding: 16px; }}
    h2 {{ margin: 0 0 12px; font-size: 18px; }}
    ul {{ margin: 0; padding-left: 18px; }}
...
<main>
  ...
  <table>
    <thead><tr><th>卡片</th><th>图片</th><th>总分</th><th>等级</th><th>主要扣分项</th><th>维度分</th><th>详情报告</th><th>警告</th></tr></thead>
    <tbody>{rows}</tbody>
  </table>
</main>
</body>
...
def render_batch_row(result: EvaluationResult, output_dir: Path) -> str:
    ...
    deduction_text = "；".join(f"{item.reason} {item.score_delta:.1f}" for item in result.deductions[:3])
    if len(result.deductions) > 3:
        deduction_text += f"；另有 {len(result.deductions) - 3} 条"
    ...
    return (
        ...
        f"<td>{html.escape(deduction_text or '无')}</td>"
...
def render_frequent_deductions(results: list[EvaluationResult]) -> str:
    counter: Counter[str] = Counter()
    reason_by_code: dict[str, str] = {}
    for result in results:
        for item in result.deductions:
            counter[item.code] += 1
            reason_by_code.setdefault(item.code, item.reason)
    if not counter:
        items = "<li>暂无明显共性扣分项。</li>"
    else:
        items = "".join(
            f"<li>{html.escape(reason_by_code.get(code, code))}：{count} 次</li>"
            for code, count in counter.most_common(8)
        )
    return f"<section><h2>常见扣分项</h2><ul>{items}</ul></section>"


def render_frequent_hints(results: list[EvaluationResult]) -> str:
    counter: Counter[str] = Counter()
    for result in results:
        counter.update(result.prompt_suggestions)
    if not counter:
        items = "<li>暂无明确批量提示词优化方向。</li>"
    else:
        items = "".join(f"<li>{html.escape(hint)}：{count} 次</li>" for hint, count in counter.most_common(8))
    return f"<section><h2>提示词优化方向</h2><ul>{items}</ul></section>"


def render_deductions(result: EvaluationResult) -> str:
    if not result.deductions:
        body = "<p>无明显扣分项。</p>"
    else:
        rows = []
        for item in result.deductions[:12]:
            components = ", ".join(item.component_ids) if item.component_ids else "无"
            rows.append(
                "<div class=\"deduction\">"
                f"<strong>{item.score_delta:.1f}</strong>"
                f"{html.escape(item.reason)}"
                f"<small>来源：{html.escape(item.source)}；严重度：{html.escape(item.severity)}；组件：{html.escape(components)}</small>"
                f"<small>证据：{html.escape(item.evidence or '无')}</small>"
                f"<small>修复建议：{html.escape(item.fix_hint or '无')}</small>"
                "</div>"
            )
        body = "".join(rows)
    return f"""<section class="deductions">
      <h2>主要扣分项</h2>
      {body}
    </section>"""


def render_prompt_suggestions(result: EvaluationResult) -> str:
    items = "".join(f"<li>{html.escape(item)}</li>" for item in result.prompt_suggestions) or "<li>本样本暂无明确提示词优化建议。</li>"
    return f"""<section class="deductions">
      <h2>下一轮生成建议</h2>
...
def render_hard_caps(result: EvaluationResult) -> str:
    if not result.hard_caps:
        return ""
    caps = "；".join(f"{item['code']} <= {float(item['cap']):.0f}" for item in result.hard_caps[:3])
    return f"<p>封顶：{html.escape(caps)}</p>"
