...
def test_extracts_static_path_and_format_string(tmp_path: Path) -> None:
    ...
    assert " " not in texts
    assert info.root_id is None or isinstance(info.geometry_boxes, list)
...
def test_infers_root_and_geometry_boxes_without_update_root(tmp_path: Path) -> None:
    dsl = tmp_path / "geometry.jsonl"
    dsl.write_text(
        '{"version":"v0.9","updateComponents":{"surfaceId":"s","components":['
        '{"id":"root","component":"Column","children":["title"],"styles":{"width":160,"height":160,"padding":12}},'
        '{"id":"title","component":"Text","content":"25℃","styles":{"width":60,"fontSize":24,"maxLines":1}}'
        ']}}',
        encoding="utf-8",
    )
    ...
    assert info.surface_width == 160
    assert info.surface_height == 160
    assert any(box.component_id == "title" and box.text == "25℃" for box in info.geometry_boxes)
